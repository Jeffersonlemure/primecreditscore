...
async function testarConsultas(token) {
  ...
  const urlBasicaPJ = `${BASE_URL}/crednet/pjconsultation/${HOMOLOG_CNPJ}/${HOMOLOG_UF_PJ}/RELATORIO_BASICO_PJ?optionalFeatures=SCORE_POSITIVO`
  const rBasicaPJ = await consultar(token, 'Básica PJ  (RELATORIO_BASICO_PJ + SCORE_POSITIVO)', urlBasicaPJ)
  if (rBasicaPJ) {
